...
- [ ] 【優先・Phase 4着手前】review.ts・delete.ts・workflow.tsの本番SQLを実SQLiteで実行する結合テストを整備する。今回の`ambiguous column name`はレビューで記録した「SQL文字列モック・手書きコピー検証の限界」がそのまま本番障害化したもの。SQL文をエクスポートしてテストから同一文字列を実行できる構造へ変更する。
...
- [ ] 実機確認（ユーザー実施）: `LITTLE_ECHOES_DEVICE_TOKEN`を設定して`uv run python main/apps/pc-client/src/client/app.py`でGUIを起動し、長押し録音→送信→確認待ち表示、未満解放の取り消し、未送信再試行を確認する。送信検証には書き込み有効化が必要（要指示）。固定サンプル送信は`main/apps/pc-client/src/assets/sample.wav`の配置後に有効。
- [ ] 実機確認: GUI起動、マイクでの長押し録音→送信→確認待ち表示、固定サンプル送信、切断・スプール満杯時の表示（ユーザー実施。トークンは`.tools/secrets/device-token.txt`、書き込み有効化が必要な場合は要指示）。
