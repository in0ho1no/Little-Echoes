...
| 2 — 固定データ縦断スライス | ほぼ完了（実環境縦断まで確認済み） | 実デプロイ・実D1・縦断E2E・キルスイッチ実測済み。書き込みは`DEMO_WRITE_ENABLED=false`へ戻し済み。残: 隔離運用手順、スマホ実機再生、Fable5第2回 |
...
- [ ] スマートフォン実機で管理画面の音声再生を確認する（音声取得はHTTP Range未対応のため、iOS Safariでの再生可否を確認し、必要ならRange対応を追加する）。
- [ ] Fable5レビュー第2回（今回の収束実装とデプロイ差分を対象）を実施し、残Minor（`retries.limit`意味論の公式確認、相関ID接頭辞`corr_`/`cor_`の整合、削除E2Eの実環境確認）を消化する。
