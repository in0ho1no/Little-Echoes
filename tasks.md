...
- [ ] 生成上限・残り回数をダイアログと画面に表示する（ユーザー要望 2026-07-30）。サーバーは全数値を保持済み: 録音別画像通算（`usage_counters`の`recording:<id>:image`、上限5）、画像日次（20）、テキスト日次（100）、手動再生成の使用済みフラグ。日記詳細API応答へ残り回数フィールドを追加（openapi更新）し、画像生成・置換の確認ダイアログへ「この録音の残り生成回数 n/5」、画面へ日次残を表示する。Phase 7.5で実施。
...
- [ ] 生成上限・残り回数の表示（上記UI改善バックログの詳細どおり。API応答の拡張とopenapi更新、SPECの絵日記UI要件への追記を実装前に行う）。
