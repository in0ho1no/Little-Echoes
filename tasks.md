...
| 1A — PC音声スパイク | 実機確認待ち | クラウド非接続の実装・自動品質確認・Solレビュー済み。Fable5レビューとデモPCのマイク確認が残る |
...
- [ ] Fable5レビューを実施し、指摘があれば修正・再検証する。現環境では利用不可のため未実施。
- [ ] デモPCで`uv run python main/apps/pc-client/src/audio/spike.py`により入力デバイスを確認し、`uv run python main/apps/pc-client/src/audio/spike.py --record <保存先.wav>`で24 kHzまたは48 kHzフォールバックのWAV保存を確認する。結果（選ばれた形式、切断時の挙動、必要な仕様更新）を`SPEC.md`へ反映する。
