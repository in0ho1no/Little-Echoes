...
| 4 — PC参照クライアント | 実装・独立レビュー修正済み、実機確認待ち | 固定サンプル配置と実機E2E、書き込み有効化は未実施 |
...
- [ ] 実機確認（ユーザー実施）: `LITTLE_ECHOES_DEVICE_TOKEN`を設定して`uv run python main/apps/pc-client/src/main.py`でGUIを起動し、長押し録音→送信→確認待ち表示、1.5秒未満の取り消し、マイク切断時の途中保存と1回再接続、未送信再試行を確認する。送信検証には書き込み有効化が必要（要指示）。固定サンプル送信は`main/apps/pc-client/src/assets/sample.wav`の配置後に有効。
