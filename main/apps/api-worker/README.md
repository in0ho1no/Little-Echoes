...
- 画像生成のOpenAI要求タイムアウトは120秒（テキスト系30秒と分離）。cronは日次`17 3 * * *`（保持期限削除・R2孤児スイープを含む全量）と毎時`47 * * * *`（日記・画像・cleanupの再調停のみ）の2本で、非終端ジョブは最長1日以内に終端へ収束する
- R2孤児スイープは`diary-images/`配下のうち、作成から24時間以上経過しD1参照（有効`diary_images`行・非終端ジョブ）のないオブジェクトだけを日次で有限件数削除する（purge後に遅延着地したputの回収経路）
