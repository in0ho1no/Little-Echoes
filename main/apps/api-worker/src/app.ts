...
app.get('/recordings/:id', async (c) => {
  ...
  // scriptタグは補間を含むテンプレートへ入れない — 全値はescapeHtml済みだが、同一文字列に
  // 混在させるとXSS監査（unknown-value-with-script-tag）の検出対象になるため分離する。
  const pageScripts = '<script src="/assets/review-detail.js"></script><script src="/assets/review-remove.js"></script>';
  return c.html(`<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Little Echoes — 録音</title></head><body><main data-recording-id="${recording.id}"><p><a href="/">確認待ち一覧へ戻る</a> · <a href="/diary">絵日記</a> · <a href="/dictionary">ことば辞典</a></p><h1>録音の確認</h1><p><strong>状態:</strong> ${escapeHtml(recording.analysis_status)}</p><p id="processing-status">${status}</p><p><strong>録音日時:</strong> ${escapeHtml(recording.captured_at)} (${escapeHtml(recording.captured_timezone)})</p><audio controls preload="metadata" src="/api/v1/recordings/${recording.id}/audio">このブラウザでは音声を再生できません。</audio><h2>文字起こし</h2><p>${escapeHtml(transcript?.reviewed_text ?? transcript?.raw_text ?? 'まだありません。')}</p><h2>単語候補</h2><ul>${candidateList}</ul>${editor}</main>` + pageScripts + '</body></html>');
});
