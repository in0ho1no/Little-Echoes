...
1. リポジトリの公開方針を確定する（Public化またはPrivate+collaborator追加。本リポジトリを継続使用） → Publicで作成済み。
2. `reference/`を実行時依存へ含めない方針をREADMEへ記載
3. 既存VoiceInteractionGadgetのURLまたは基準コミットを参考情報として記録  
    - faba8a0fef600bcb971e4df717e53b326ab748e7までが過去実装やテンプレ環境の取り込み
    - faba8a0fef600bcb971e4df717e53b326ab748e7以降がLittle-Echoes向けの開発
4. Pre-existing referenceとBuild Week中の新規開発範囲をREADMEへ明記
5. `SPEC.md`をコミット → 本リポジトリを使うのでコミット済み
6. Codexの主要作業セッションを開始 → 開始済み
...
3. Recording、Transcript、WordCandidate、WordOccurrence、DiaryEntry、DiaryImage、DictionaryWord、ProcessingAttempt、AsyncJob、UsageCounterを定義
4. OpenAPIと共通エラースキーマを定義
5. D1スキーマ、一意制約、外部キー、トランザクション境界を定義
6. 非公開R2キー構成と保持・削除方針を定義
7. 管理用ホストとデバイス用ホストの経路、Cloudflare Access JWT検証、デバイストークン認証・認可を定義
8. WorkflowのジョブID、状態、明示的な再試行設定、D1との整合規則を定義
9. 冪等性、楽観ロック、試行上限、日次コスト上限、有効期限、緊急停止を定義
10. OpenAI APIの`store: false`、background mode不使用、データ開示要件を定義
11. サンプルJSONとAPI契約テストを作成

### Phase 1A: PC音声スパイク

Phase 1と並行し、半日を上限としてクラウド非接続の単体スクリプトを作る。`sounddevice` 0.5.5は承認済みで`uv add`により追加し、`uv.lock`で固定している。
...
1. Worker、Workflows、D1、R2プロジェクト作成
2. Workflows要件を満たす`compatibility_date`をWrangler設定へ固定
3. 管理用ホストとデバイス用ホストの分離、認証・認可
4. Cloudflare Access JWTの署名、`iss`、`aud`、`exp`検証
5. 固定WAVアップロード
6. R2保存とD1記録作成
7. モックWorkflowの受付、`202 Accepted`、状態ポーリング
8. モック文字起こし・単語候補
9. 状態取得と認可付き音声再生
10. 確認待ち一覧と詳細画面
11. 状態表示とエラー表示
12. 同じ`client_capture_id`と`AsyncJob.id`の重複排除
...
2. 単語候補の構造化出力
3. ユーザーデータを命令から分離
4. Responses APIで`store: false`を強制し、OpenAI background modeを使用しない
5. Workflowの明示的な再試行設定、OpenAI SDKの`maxRetries: 0`、`NonRetryableError`分類
6. JSON Schema検証
7. `partial`フォールバック
8. AsyncJobとProcessingAttempt記録
9. 試行上限と日次上限
10. 固定サンプルでの実API確認
...
1. 承認済みデータから日記文生成
2. 日記文生成Workflowの受付、`202 Accepted`、状態ポーリング
3. 日記文の保存・手動編集
4. 明示操作による画像生成Workflow
5. 有効画像1枚の管理と再生成による置換
6. 置換前の確認ダイアログ
7. 生成失敗時に既存画像を維持
8. 通算・日次生成上限
...
2. ホスト別ルート、Access JWT検証、審査員アドレス完全一致ポリシーの確認
3. コスト上限、Workflowの有限再試行、2026-09-01の有効期限、`DEMO_WRITE_ENABLED`の確認
4. 固定サンプル音声と期待データ
5. 主要フローの自動テスト
6. 保持期限と削除処理
7. `reference/`なしのビルド・テスト・デモ
8. デモ用データ初期化手順
9. 審査員向け実行手順
10. OpenAI APIのデータ取り扱いをREADME、Project Story、Testing instructionsへ英語で記載
11. エラー時の復旧手順
12. 公開読み取り専用デモまたは認証済みテスト環境
13. デモ動画撮影
...
- 48 kHzから24 kHzへの変換
- WAVヘッダー
- スプールWAVのアップロード確認後削除、`/process`受付までの最小メタデータ保持、失敗時保持
- スプール件数・容量・保持期限
- API入力検証
- 状態遷移
- 禁止された状態遷移
...
- 同じ`AsyncJob.id`のWorkflow作成が冪等であり、dispatch結果不明時にも別ジョブを作らない
- Workflowの各`step.do()`が既定の`retries.limit = 5`を使わず、本書の合計試行回数を明示する
- OpenAI SDKの`maxRetries`が0で、Workflow外の暗黙再試行が発生しない
- 恒久エラーが`NonRetryableError`相当として再試行されず、過渡障害だけが設定回数内で再試行される
- 要求送信後の結果不明タイムアウトが自動再試行されず、`UPSTREAM_RESULT_UNKNOWN`となる
- Workflowのペイロードと永続ステップ出力に音声、文字起こし、親メモ、トークンが含まれない
- 古い`active_attempt_id`の遅延結果が新しい試行結果を上書きしない
- 長時間更新のない非終端`AsyncJob`がWorkflow状態との照合で終端状態へ収束する
- 楽観ロック競合
- 単語正規化
- 初出判定
- 過去日時の後日承認
...
- 同時承認時の辞典整合性
- 試行回数と日次コスト上限
- 上限到達で解析を開始できない録音の`failed`遷移
- 解析再試行が文字起こしから再実行されること
- Responses API要求が`store: false`であり、OpenAI background modeを使用しないこと
- `captured_at`の2000〜2099年範囲検証
- 有効画像1枚と再生成による置換
- 削除時の関連データ処理

### 結合テスト

- サンプルWAV → Cloudflare → Workflow受付`202 Accepted` → 状態ポーリング → モック文字起こし
- 同じ`client_capture_id`の再送 → 同じ録音ID
- 同じ解析開始要求の再送 → 同じ`AsyncJob`とWorkflow
...
- コスト上限で解析を開始できない録音 → `failed` → 手動入力で承認
- 空の`reviewed_text`承認 → 場面とメモから日記文生成
- 承認 → 辞典登録
- 二重承認 → 発話回数が増えない
- 承認 → 日記Workflow受付`202 Accepted` → 状態ポーリング → 日記生成
- 画像生成 → R2保存 → Web表示
- 画像保持後の再生成失敗 → 既存画像を維持
- 画像保持後の再生成成功 → 確認済みの既存画像だけ置換
- 画像生成の同時要求 → 1件だけがOpenAI APIを呼ぶ
- 画像失敗 → 日記と音声は閲覧可能
- 削除 → Workflow受付`202 Accepted` → 即時非表示 → D1/R2両方の削除
...
- 期限切れ審査用トークンの拒否
- `DEMO_WRITE_ENABLED = false`でデータ作成・更新・AI処理を拒否し、認可済み削除と後始末は継続できること
- デバイストークンによる管理APIアクセスの拒否
- 管理用ホストへのデバイストークン要求と、デバイス用ホストへの管理API要求の拒否
- Access JWTの署名、`iss`、`aud`、`exp`不正の拒否
...
- OpenAI要求とWorkflow永続状態へ仕様外のユーザーデータが含まれないこと
...
- Devpostの審査員限定Testing instructionsに、Cloudflare Accessで許可する`testing@devpost.com`と`build-week-event@openai.com`、メールワンタイムPINの操作、別アドレスを使う場合の連絡先を英語で記載する。ドメイン全体を許可しているとは書かない
- READMEまたはProject Storyに、管理Web/管理APIとデバイスAPIを別ホストへ分離し、Cloudflare Accessは管理用ホストだけに適用する設計を英語で記載する
- デモ用デバイストークンの値はPublicリポジトリや公開READMEへ記載せず、審査員限定Testing instructionsで安全な入力方法とともに案内する
- Testing instructionsに、審査用トークンと書き込みデモが結果発表後に前倒し停止され、遅くとも2026-09-01 00:00 JSTに失効することを英語で記載する
- README、Project Story、Testing instructionsに、音声は文字起こし、確認済みテキスト・単語・場面・親メモは文章生成、承認済み日記内容は画像生成のためOpenAI APIへ送信されることを英語で記載する
- 同じ3提出物に、APIデータは既定で学習に使われないこと、適用される不正利用監視ログではコンテンツが最大30日保持され得ること、Responses APIへ`store: false`を指定するがゼロ保持を意味しないこと、実在する子どものデータを使わないことを英語で記載する
- 公開前チェックリストに、公開リポジトリへトークンがないこと、Access許可が名指しアドレスだけであること、期限・緊急停止が機能すること、上記データ取り扱い説明が3提出物すべてにあることを含める
