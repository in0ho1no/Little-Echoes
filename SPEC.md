...
本書は、**Little Echoes** の要件、構成、開発順序、受け入れ条件を定義する。
...
Little Echoesでは、この実績を技術的な参考情報として利用しつつ、次の機能を独立した製品コードとして開発する。

- 直前音声を保存するリングバッファ
- 実機を持たない利用者も検証できるPC参照クライアント
- Cloudflare上のバックエンド
- スマートフォン向け確認Webアプリ
- 文字起こし、単語抽出、初出判定
- 絵日記文と画像の生成
...
| 一時デモ環境の書き込み有効期限 | 2026-09-01 00:00 JSTより前。運用終了時は前倒し停止 |
...
- 一時デモ用デバイストークンの`expires_at`とデモ環境の`DEMO_WRITE_ENABLED`を、データ作成・更新・AI処理APIで確認する。認可済みの削除と期限切れデータの後始末は停止対象にしない
- 2026-09-01 00:00 JST（2026-08-31 15:00 UTC）以降は、日次上限に残余があっても一時デモ用トークンによる要求と新しいAI処理を拒否する。期限前に受け付けたWorkflowも、OpenAI API呼び出し直前に期限と緊急停止を再確認する
...
- クライアント側は最小限のvanilla JSとし、独立したSPA用ビルドパイプラインを追加しない。TypeScript WorkerのWranglerによるビルド・バンドルは行う（根拠は依存最小・保守性と同一オリジン構成の維持。2026-07-31の決定を参照）
...
MVPでは操作前10秒・操作後5秒を固定値とする。秒数の設定変更は将来対応とし、D1とAPI契約の検証上限（`pre_roll` 0〜10、`post_roll` 0〜5、クリップ20秒以下）と一致させる。
...
クリップ確定後のレベル補正: 録音環境によっては入力レベルが小さいため、基準形式PCMへ変換した後に上限付きの増幅を適用してよい。最大8倍、目標ピーク27,000とし、ピーク256未満の無音相当は増幅しない。目標を超える突発音はソフトリミットで16-bit範囲へ収める。補正はクリップ確定後だけに行い、音声コールバック内では行わない。第一の対策はOSの入力レベル適正化とし、補正はその補助とする。

### 音量補正

- PCクライアントはクリップ確定後、音声コールバック外で、絶対値256以上の有音PCMだけの95パーセンタイルを確認する。短い一語が無音に埋もれず、Enterキー音など単発の大きな音を発話の音量推定へ過度に反映しない
- 小さい発話は最大8倍まで増幅し、目標ピークを約-1.7 dBFS（27,000）に制限する。目標を超える突発音はソフトリミットし、16-bit範囲を超える値は出力しない
- ピークが256未満の無音・極小ノイズは増幅せず、既に目標以上のクリップも減衰させない
- OSの入力デバイス選択・マイク入力レベルを優先し、音量補正は録音音量が小さい場合のローカルな補助とする
...
| `ready` | 日記文を閲覧・編集可能 | `ready`, `generating` |
| `failed` | 日記文生成失敗。承認済み記録は閲覧可能 | `generating`, `ready` |

承認時に空の`DiaryEntry`を先に作成する。日記生成が失敗またはコスト上限へ到達した場合も、親は日記文を手動入力して`ready`にできる。`ready -> generating`は親の明示再生成操作だけに使用する。
...
| `ready` | 有効な画像を1枚保持 | `generating`, `not_requested`, `limit_reached` |
| `failed` | 画像がなく、直近生成が失敗 | `generating`, `limit_reached` |
| `limit_reached` | 1録音あたり通算5回の生成上限到達 | 終端 |

既存画像がある状態で再生成に失敗した場合は`ready`を維持し、`last_generation_error`だけを更新する。`ready -> not_requested`は画像削除操作だけに使用する。画像状態は日記の公開可否へ影響させない。日次上限は同期APIの事前確認で拒否する場合、状態を変更せず`COST_LIMIT_REACHED`を返す。事前確認をすり抜けてWorkflow内の予約で到達した場合も`generating`のまま残さず、ジョブを`COST_LIMIT_REACHED`で終端して日記は`failed`、画像は既存画像があれば`ready`・なければ`failed`へ収束させる。この収束では試行予算・通算回数・手動再生成権を消費しない（予約トリガーの中止で原子的に不成立となるため）。いずれの場合も翌UTC日には再度操作できる。録音単位の通算上限だけを`limit_reached`として永続化する。
...
- 意味内容による自動禁止語リストは設けない。固有名詞や幼児語を削除しすぎず、候補の採否は親のレビューを正とする
- 空文字・空白だけの候補、NULなどの制御文字を含む候補、文字数・件数・Schema上限に違反する出力は保存せず、文字起こしを保持した`partial`へ遷移する。候補配列が空の場合も同様とする
...
- データベース照合を正とする
- 未承認記録は初出確定に含めない
- 削除中（`deleting`/`delete_failed`）の録音の発話も辞典集計（件数・初出・`is_first`）に含めない。承認済み（`approved`）だけを数える。参照が残る辞典単語行は件数0でも削除せず、参照消滅後の再計算で削除する
...
- OpenAI APIプロジェクトでデータ共有へ明示的にオプトインしない。Zero Data Retentionの承認を受けていない限り、公開文書でゼロ保持とは表現しない
- デモでは実在する子どもの音声・画像・個人情報を使用しない。将来実データを扱う前に、同意、プライバシー表示、保持、削除、上流サービスのデータ管理を改めて承認する
- READMEまたはプライバシー文書へ、OpenAI APIへ送るデータ、学習利用の既定、保持可能性、`store: false`、実在児童データ不使用を明記する
...
`household_id`、`source_type`、`source_id`、`captured_at_source`はクライアントから受け取らず、認証済みデバイストークンからサーバーが導出する。

サーバー検証:

- 認証されたデバイスが所属する`household_id`をサーバー側で付与する
- `source_type`と`source_id`はデバイストークンに束縛された値をサーバーが導出し、クライアント指定を受け付けない
...
- `captured_at_source`は認証済みデバイス種別からサーバーが決定する。`manual`は確認画面の日時編集だけで設定され、アップロード時には設定されない
...
非終端の`AsyncJob.updated_at`が処理種別ごとの想定時間を超えて古い場合だけ、サーバーが対応するWorkflowインスタンスの状態を照合する。想定時間は解析15分、削除24時間とする。正常に実行中なら`updated_at`を更新して処理中表示を維持し、Workflowが終了済み（ジョブが非終端のままなら結果は未コミット）の場合は実行中attemptを失敗として終端し、ジョブと`analysis_status`を`failed`へ収束させて、エラーコードと相関IDを返す。この収束は同一原子バッチ内のジョブ終端化が成立した場合にだけ確定させ、並行して完了した成功結果を上書きしない。照合自体が不明な場合は状態を変更せず、次回ポーリングで再照合する。`dispatch_pending`のまま想定時間を超えたジョブは、同じ`AsyncJob.id`でWorkflow作成または既存確認を再実行して収束させる。エラー応答の`retryable`はクライアントの自動再送可否を表し、収束済みエラーでは慎重側の`false`とする。手動再試行の可否は試行予算（解析APIの上限判定）と`next_action`の文言で示す。
...
- 楽観ロックが不成立の場合はトランザクション全体を中止し、同一バッチ内のどの文もデータへ到達させない。「現在のversionが期待値+1に一致する」ことを後続文の成立条件として使わない（同じ基底versionで競合した敗者の書き込みが、勝者の作った同じ値に一致して着地するため）
...
- Phase 3の承認APIは、確定文字起こし・辞典・`DiaryEntry`下書き（`diary_status = not_started`）までを同期的に確定し、成功時は`200 OK`を返す。日記用`AsyncJob`、Workflow、日記文生成はPhase 6で実装する
- Phase 3では`generate_image`を受け取らず、画像生成要求も開始しない。日記文・画像の生成失敗、手動入力、有限再試行はPhase 6の責務とする
...
- 画像生成のOpenAI要求タイムアウトは120秒とし、テキスト系（文字起こし・単語抽出・日記文）の30秒とは分離する。タイムアウト時の扱いは`UPSTREAM_RESULT_UNKNOWN`の規則に従う
...
- 画像がない場合は確認後に生成できる
- 画像を保持している場合は、現在画像のサムネイルと作成日時を確認ダイアログへ表示する。サムネイルを取得できない場合は、画像を確認できない旨のメッセージを代わりに表示する
- ダイアログには「新しい画像の生成に成功した場合だけ既存画像を置き換える」と明記する
- 日記詳細APIは、D1の利用量を正として、録音別画像生成、画像生成のUTC日次、画像以外のAI呼び出しのUTC日次について、それぞれ使用済み回数、上限、残り回数を返す。残り回数は`max(0, 上限 - 使用済み)`で算出し、日記文の手動再生成が使用済みかどうかも返す
- 日記詳細画面は「この録音の画像生成」「本日の画像生成」「本日のテキスト系AI」の残り回数を表示する。日次回数がUTC日単位であることと、テキスト系AIには文字起こし・単語抽出・日記文生成が含まれることを明記する
- 初回画像生成と画像置換の確認には、開始前の「この録音の残り生成回数 n/5」と本日の画像生成残数を表示する。表示値が別画面の操作で古くなっていても、生成開始時のサーバー側上限判定を正とし、UI表示だけで上限を判断しない
- 生成中は二重送信を防止し、進捗表示とキャンセル不能である旨を表示する。進捗表示は経過時間に応じて段階的に変化させ、処理が進んでいることを伝える
...
- Accessの許可ポリシーは、所有者が個別承認したメールアドレスの完全一致だけとし、ドメイン単位では許可しない
- デモ用デバイストークンはリポジトリへ含めず、承認済みの安全な経路で配布する。秘密であることだけに依存せず、権限、利用量、有効期限、失効で被害を制限する
- 一時Access許可とデバイストークンは不要になった時点で前倒しして失効させ、遅くとも2026-09-01 00:00 JSTに無効化する
- 運用手順には、許可済みアドレスの管理、ワンタイムPINの手順、デバイストークンの入力方法と有効期限を明記する
...
- 一時デモ用トークンの`expires_at`は2026-09-01 00:00 JSTより前とする。`DEMO_WRITE_ENABLED = false`ではデータ作成・更新・AI処理を直ちに停止するが、認可済みの削除と期限切れデータの後始末は許可する
...
Little Echoesは本リポジトリ（SPEC策定から継続）で開発する。リポジトリの公開範囲と共有先は、公開前に所有者が決定する。
...
READMEには、製品コードが`main/`配下にあり、`reference/`は実行時に使用しない任意の技術資料であることを明記する。
...
各Phaseの実施内容、進捗、Agenticレビュー手順、テスト実行計画、公開・運用準備、未決の実装判断は[tasks.md](tasks.md)を正とする。
...
- 公開文書にOpenAI APIのデータ取り扱いと実在児童データ不使用が明記される
- 削除操作後、対象データが通常画面へ再表示されない
- 公開デモは実データ環境と分離される
- 一時デモ用トークンと書き込み機能が2026-09-01 00:00 JSTまでに失効する
...
- 2026-09-01 00:00 JST以降は日次上限に残余があっても一時デモ用トークンからコストを発生させられない
...
| 一時デモのアクセスモデルが未定義 | 管理WebはCloudflare AccessのメールワンタイムPIN（所有者が個別承認したアドレスだけ。ドメイン許可なし）。デバイスAPIは別ホストに分離し、デモ用デバイストークンは承認済みの安全な経路で配布する。トークンと一時Access許可は不要になった時点で前倒し失効し、遅くとも2026-09-01 00:00 JSTに無効化する |
...
| 項目 | 決定 |
| --- | --- |
| OpenAIモデル | 文字起こし: `gpt-4o-transcribe`（`/v1/audio/transcriptions`へのRESTファイルアップロードで使用。音声時間単位課金で日次100回上限により有界。当初の`gpt-realtime-whisper`は実APIでREST非対応が確認されたため2026-07-28に変更）。単語抽出・日記文生成: `gpt-5.6-luna`。画像生成: `gpt-image-2`（`gpt-image-1`はdeprecated） |
...
| 一時デモアクセスの期限 | 個別承認したアドレスだけを許可し、デモ用デバイストークンとAccess許可は不要になった時点で前倒し失効する。最終期限は2026-09-01 00:00 JSTとする |
| OpenAI APIのデータ取り扱い | [OpenAI APIのデータ制御](https://developers.openai.com/api/docs/guides/your-data#default-usage-policies-by-endpoint)を確認し、Responses APIは`store: false`、OpenAI background modeは不使用とする。既定で学習へ使われないことと、適用される不正利用監視ログでは最大30日保持され得ることを公開文書へ明記し、ゼロ保持とは表現しない |
...
### 2026-07-21 Phase 1A実機確認

| 項目 | 決定 |
| --- | --- |
| デモPCのキャプチャ形式 | 24 kHz直接入力に確定（既定入力デバイスAudioBox Goで24 kHz/48 kHzとも`check_input_settings`の受理を実測）。48 kHzフォールバックはテスト済みコードとして維持する |
| 入力レベルの補正 | 実機で録音音量が小さかったため、クリップ確定後の上限付き増幅（最大8倍・目標ピーク27,000・ピーク256未満は非対象・突発音はソフトリミット）を採用。OS入力レベルの適正化を第一とし、補正は補助とする |
| CLIでの長押し模擬 | コンソールの`input()`では物理的なEnter長押しを検出できないため、スパイクは「Enter後に固定1.5秒待機で確定」とする。製品GUIはpress/releaseイベント+1.5秒タイマーで実装する |

### 2026-07-22 Phase 3の承認と日記生成の分離

| 項目 | 決定 |
| --- | --- |
| 承認APIの範囲 | Phase 3では、親が確定した文字起こし・単語・録音日時・場面・親メモ、辞典集計、空の日記下書きを一貫して保存する。日記生成Workflowは作成しない |
| 応答 | 承認のD1確定後に`200 OK`を返す。`202 Accepted`は外部AIを伴うPhase 6の日記・画像生成でのみ使用する |
| 背景 | 承認・日時編集・辞典初出の整合性を先に単独で検証し、日記生成の外部API・コスト上限・結果不明処理を混在させない。親の承認済みデータはAI処理失敗の影響を受けず、Phase 6で安全に非同期生成を追加できる |

### 2026-07-23 Phase 5文字起こしモデルの確認
...
| 呼び出し予約とUTC日 | 転写と単語抽出を別々の呼び出し予約としてD1へ記録し、UTC日をSQLiteで正規化して日次100回を超える予約を原子的に拒否する。予約済み呼び出しだけを実行し、結果不明時は自動再送しない |
| 部分成功と長文 | 空の転写または空の単語候補は`partial`として親の手動補完へ渡す。転写が2,000文字を超える場合は`TRANSCRIPT_TOO_LONG`で`failed`とし、切り詰めて単語抽出へ送らない |
| 明示的な再解析 | 管理画面は`partial`または`failed`かつ未承認、解析試行3回未満、有効な入力元トークンあり、非終端解析ジョブなしの場合だけ再解析を表示する。予約時に`analysis_status = pending`へ戻し、旧画面の保存を楽観ロックで拒否するため`version`を増加させる |

### 2026-07-31 Webフロントエンド構成の根拠更新

| 項目 | 決定 |
| --- | --- |
| 制約の維持 | 2026-07-20決定の「独立したSPA用パイプラインは追加しない」を維持する。ただし根拠から短期実装の優先を外し、依存最小・保守性と、同一オリジン+Cloudflare Access一括保護の2点を存続根拠とする |
| 位置づけの変更 | 不可侵の制約ではなく、コスト対効果で再検討可能な決定とする。SSR+vanilla JSで実現できない具体的なUX要件が生じた場合に限り再検討し、その場合も同一Workerからの配信により同一オリジン構成を維持する。切り替え時は本書を先に更新して承認を得る |
| 背景 | ハッカソン提出の取りやめとPhase 7完了により、短期実装を優先する根拠は失効した。一方、Workerの実行時依存が少数（hono, jose, openai, zod）に保たれている状態は、Phase 7で固めたセキュリティ姿勢（サプライチェーン面の攻撃対象領域の抑制）と個人運用での保守性を支えるため、制約自体は維持する |

### 2026-07-31 公開判断

| 項目 | 決定 |
| --- | --- |
| 公開範囲・共有先 | 本リポジトリをGitHubでpublic公開する。共有先の限定は行わない |
| `reference/`の扱い | 参照実装はPhase 8で利用するため公開時点ではツリーに残し、Phase 8完了後にフォルダごと削除する。git履歴に残ることは許容する（履歴書き換えは行わない） |
| OSSライセンス | MIT License。著作権者表記は `in0ho1no`（GitHubアカウント名。実名は公開しない）。依存パッケージのライセンス表示（Zod=MIT、OpenAI SDK=Apache-2.0、Hono=MIT）は別途維持する |
| スクリーンショット | 公開物に使う場合は、実データ・メールアドレス・内部ID・相関IDが含まれないことを撮影時に確認する |
| デモ環境 | 現状維持。Cloudflare Access保護（個別承認アドレス1件）の非公開デモを2026-09-01期限で継続し、別環境の公開読み取り専用デモは作らない |
| 実施順序 | LICENSE追加とスクリーンショット安全確認の後、CI Semgrep/gitleaks外部ゲートの成功を確認してからpublicへ切り替える。public切替・Secret操作はユーザーが明示的に実施する |

---

## 完成の定義

Little EchoesのMVPは、次の状態をもって完成とする。

> 実在する子どものデータを使用せず、PC参照クライアントで発話前の音声を遡って安全に保存し、有限な試行回数とコスト上限のもとでCloudflare WorkflowsとOpenAI APIにより非同期処理し、スマートフォン上で状態を確認・修正・承認した後、初出単語付きの絵日記として閲覧できる。管理用・デバイス用の経路と認証を分離し、OpenAI APIのデータ取り扱いを公開文書へ明記する。失敗時もデータを失わず理由と復旧操作が示され、固定サンプル音声を使って`reference/`なしで同じ体験を再現でき、一時デモアクセスと書き込みは不要になった時点、遅くとも2026-09-01 00:00 JSTまでに停止される。
