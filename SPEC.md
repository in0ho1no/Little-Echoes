...
- クライアント側は最小限のvanilla JSとし、SPAフレームワークとビルドステップを追加しない
- 同一オリジン構成によりCORS設定を不要にし、Cloudflare Accessの保護を1つのWorkerへ適用する
...
- AI出力を確定データとして扱わない
- 固有名詞や幼児語を削除しすぎない
- 使用モデルは`gpt-5.6-luna`とし、JSON Schemaによる構造化出力を使用する
...
- 生成サイズは1024×1024、品質は最低設定とする。絵日記風の画風には低品質で十分であり、コストも抑えられる
- 品質パラメータの具体的な指定値は実装開始時にAPIリファレンスで確認する
- 親が明示的に選択した記録のみ
- 実在する子どもの顔や容姿を再現しない
- 架空の子どもとして生成する
- 日記内容を象徴する絵日記風のイラスト
...
- 受理時は`202 Accepted`と現在の`analysis_status`を返し、完了を同期応答で待たせない。結果は状態取得APIのポーリングで確認する
...
- 承認APIはD1トランザクション完了時点で応答を返し、日記文生成・画像生成は非同期に実行する。Web画面は状態のポーリングで進捗を表示する（解析開始APIと同じパターン）
- 承認完了後に日記文生成を開始する
...
- Accessの許可ポリシーは、開発者本人のアドレス、`testing@devpost.com`、`build-week-event@openai.com`、`@devpost.com`ドメイン、`@openai.com`ドメインの和集合とする
- 上記以外のアドレスで審査する場合の連絡先を、提出時のテスト手順へ記載する
- 審査員向けデモ用デバイストークンはリポジトリへ含めず、Devpostの審査員限定Testing instructions欄で配布する
...
| 審査員のアクセスモデルが未定義 | 管理WebはCloudflare AccessのメールワンタイムPIN（名指し2アドレス+対象ドメイン+開発者）。デモ用デバイストークンはDevpostのTesting instructions欄で配布し、リポジトリへ含めない。実運用のAtomも同じデバイストークン方式 |
...
| `/process`の応答セマンティクス未定義 | `202 Accepted`+状態取得APIのポーリングに確定 |
...
| 日記文・画像生成の実行方式 | 非同期で統一。承認APIはD1トランザクション完了で応答し、生成結果は状態ポーリングで取得する |
| 画像サイズ・品質 | 1024×1024、品質は最低設定。絵日記風の画風には低品質で十分でありコストも抑えられる。品質パラメータの指定値は実装開始時にAPIリファレンスで確認 |
| Webフロントエンド構成 | APIと同一Worker上のHonoによるサーバーレンダリング+最小限のvanilla JS。SPAフレームワークとビルドステップを追加しない。1つのWorkerで完結させCloudflare Accessの保護を一括適用する |
